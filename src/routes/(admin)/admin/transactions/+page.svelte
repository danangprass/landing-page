...
<div>
	<h1 class="text-2xl font-semibold text-[#f5f5f7] mb-6">Transactions</h1>
	...
	{#if data.transactions.length === 0}
		<div class="bg-[#1d1d1f] border border-[#424245] rounded-xl px-5 py-12 text-center">
			<p class="text-[#86868b] text-sm">No transactions found.</p>
		</div>
	{:else}
				...
				<table class="w-full text-sm">
					<thead>
						<tr class="border-b border-[#424245] bg-[#000]/30">
							<th class="px-4 py-3 text-left text-xs font-medium text-[#86868b] uppercase">ID</th>
							...
							<th class="px-4 py-3 text-right text-xs font-medium text-[#86868b] uppercase">Amount</th>
							<th class="px-4 py-3 text-center text-xs font-medium text-[#86868b] uppercase">Status</th>
							<th class="px-4 py-3 text-left text-xs font-medium text-[#86868b] uppercase">Payment Method</th>
					...
					<tbody>
						{#each data.transactions as tx}
							<tr class="border-b border-[#424245]/50 hover:bg-[#2d2d2f] transition-colors">
								<td class="px-4 py-3">
									<a href="/admin/transactions/{tx.id}" class="text-[#2997ff] hover:underline font-mono text-xs">
										{tx.id.slice(0, 8)}...
									</a>
								</td>
								<td class="px-4 py-3 text-[#86868b] font-mono text-xs">
									{typeof tx.order_id === 'string' ? tx.order_id.slice(0, 8) + '...' : tx.order_id}
								</td>
		...
		{#if data.totalPages > 1}
			<div class="flex items-center justify-center gap-2 mt-6">
				{#each Array(data.totalPages) as _, i}
					<a
						href="/admin/transactions?page={i + 1}{data.currentStatus ? '&status=' + data.currentStatus : ''}"
