...
function createCartStore() {
		...
		try {
			...
			const [result] = await safeCall(() =>
				fetch('/api/cart', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ user: auth.user!.id, product: productId, quantity }),
				}).then((res) => {
					if (!res.ok) throw new Error('Failed to add item');
					return res.json();
				})
	...
	async function remove(cartItemId: string) {
		...
		const [, err] = await safeCall(() =>
			fetch(`/api/cart/${cartItemId}`, { method: 'DELETE' }).then((res) => {
	...
	async function clear() {
		...
		const results = await Promise.allSettled(
			snapshot.map(i => fetch(`/api/cart/${i.id}`, { method: 'DELETE' }).then(r => r.json()))
		);
